...
if ($_GET['department']) {
    ...
    $stmt_backup = $conn->prepare("SELECT name, regno, votepolling,departmentname FROM voterlist WHERE shift='II'");
    ...
    $reset_stmt = $conn->prepare("UPDATE voterlist SET votepolling = 0 WHERE shift='II'");
    if ($reset_stmt === false) {
    ...
    header('Content-Disposition: attachment; filename="sII_backup.csv"');
